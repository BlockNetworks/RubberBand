...
class RubberBand implements Plugin
{
    ...
    {
        $this->config = new Config($this->server->api->plugin->configPath($this) . "config.yml", CONFIG_YAML, ["api-key" => "API_KEY",
                                                                                                               "proxy-address" => "",
                                                                                                               "proxy-port" => 19132,
        ...
        if ($this->config->get("api-key") === false || $this->config->get("api-key") == "API_KEY") {
        ...
        if ($this->config->get("server") === false || $this->config->get("server") == "UNIQUE_IDENTIFIER") {
            console("[ERROR] [RubberBand] Server not set. RubberBand won't work without setting it on the config.yml");
            return;
        }        

        if ($this->config->get("proxy-address") === false || $this->config->get("proxy-address") == "") {
        ...
        PacketSendEvent::register([$this, "sendPacketProxy"], EventPriority::HIGHEST);
        PacketReceiveEvent::register([$this, "controlPacketHandler"], EventPriority::HIGHEST);
        $this->server->addHandler("server.close", [$this, "onServerStop"], 10);

        console("[INFO] [RubberBand] Connecting with frontend proxy [/{$this->address}:{$this->port}]...");
        $this->server->schedule(20 * 10, [$this, "scheduler"], [], true);
    ...
    public function controlPacketHandler(PacketReceiveEvent $event)
    {
        ...
        if (strlen($payload) == 0 || md5($payload . $this->apiKey, true) != $md5sum) { //Packet validity check
            return false;
        }
    ...
    {
        if ((time() - $this->lastConnection) > 30 && $this->connected === true) {
            $this->connected = false;
